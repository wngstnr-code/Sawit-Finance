"""Sawit Finance — AI Yield Router Agent: monitors CPO prices and SAWIT holder balances, then autonomously creates and funds yield distribution epochs on Casper Testnet."""
...
async def snapshot_sawit_holders(session: aiohttp.ClientSession) -> list[HolderBalance]:
    """Snapshot SAWIT (CEP-18) holder balances from Casper via CSPR.cloud to get proportional distribution weights."""
...
def compute_distribution(
    ...
    """Compute per-holder CSPR yield proportional to SAWIT holdings: allocation_i = total_cspr × (balance_i / total_supply)."""
...
async def submit_distribution(
    ...
    """Submit the distribution to Casper Testnet via the fund livenet bin (create_epoch + payable fund_epoch); returns the tx hash."""
